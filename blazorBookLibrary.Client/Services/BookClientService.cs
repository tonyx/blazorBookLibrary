...
public class BookClientService : IBookService
{
    ...
    private FSharpList<Book> ApplyCriteria(FSharpList<Book> books, FSharpOption<BookSearchCriteria> criteria)
    {
        if (criteria != null && FSharpOption<BookSearchCriteria>.get_IsSome(criteria))
        {
            var filter = criteria.Value;
            return ListModule.OfSeq(books.Where(b => filter.Invoke(b)));
        }
        return books;
    }

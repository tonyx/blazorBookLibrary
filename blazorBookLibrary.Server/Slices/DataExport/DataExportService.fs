...
type DataExportService
    (
        ...
        googleBooksService: IBooksMetadataSearchService,
    ...
    interface IDataExportService with
        ...
        member this.ImportFromIsbns
            (
                context: UserContext,
                isbns: List<Isbn>,
                preventDuplicates: bool,
                generateUnknownAuthors: bool,
                generateEmbeddings: bool,
                generateMissingDescriptions: bool,
                progress: IProgress<ImportProgress>,
                ...
                | Ok tenantId ->
                    try
                        for i in 0 .. total - 1 do
                            ...
                            let! bookImportResult =
                                taskResult {
                                    let! skip =
                                        if preventDuplicates then
                                            task {
                                                let! existingResult =
                                                    bookService.SearchByIsbnAsync(context, isbn, ct = ct)
                                    ...
                                    if skip then
                                        ...
                                        | Some metadata ->
                                                ...
                                                |> List.traverseTaskResultM (fun authorName ->
                                                    taskResult {
                                                        ...
                                                        if not (List.isEmpty localAuthors) then
                                                            ...
                                                            let! authorPic =
                                                                authorsSearchService
                                                                    .LookupImageUrlByNameAndThumbSizeAsync(
                                                                        context,
                                                                        authorName,
                                                                        ct = ct
                                                                    )
                                                            ...
                                                            let isni =
                                                                ...
                                                                |> Option.bind (fun s ->
                                                                    match Isni.New s with
                                                                    | Ok i -> Some i
                                                                    | _ -> None)
                                                                |> Option.defaultValue Isni.EmptyIsni

                                                            let author =
                                                                Author.NewWithOptionalIsniAndImageUrl(
                                                                    tenantId,
                                                                    name,
                                                                    isni,
                                                                    ?imageUrl = authorPic
                                                            ...
                                                            let! _ =
                                                                authorService.AddAuthorAsync(context, author, ct = ct)
                                            ...
                                            let mainCategory =
                                                matchedCategories |> List.tryHead |> Option.defaultValue Category.Other

                                            let additionalCategories =
                                                if List.isEmpty matchedCategories then
                                                    []
                                                else
                                                    matchedCategories |> List.skip 1
                                            ...
                                            let! (description: string option) =
                                                ...
                                                | Some d when not (String.IsNullOrWhiteSpace d) ->
                                                    Task.FromResult(Some d)
                                                | _ when generateMissingDescriptions ->
                                                    task {
                                                        reportProgress (
                                                            Some(
                                                                sprintf
                                                                    "Generating AI description for '%s'..."
                                                                    metadata.Title
                                                            )
                                                        )

                                                        let bookMatch =
                                                            All(isbn.Value, metadata.Title, authorsToProcess)

                                                        let! genResult =
                                                            textEmbeddingService.GetBookDescriptionAsync(
                                                                context,
                                                                bookMatch,
                                                                ct
                                                            )
                                                ...
                                                taskResult {
                                                    if generateEmbeddings then
                                                        ...
                                                        let rec getEmbeddingWithRetry (text: string) (retries: int) =
                                                            task {
                                                                let! res =
                                                                    textEmbeddingService.GetEmbeddingAsync(
                                                                        context,
                                                                        text,
                                                                        ct = ct
                                                                    )
                                                        ...
                                                        let! (embeddingResult: EmbeddingData) =
                                                            getEmbeddingWithRetry textToEmbed 3

                                                        let embeddingId = EmbeddingDataId.New()

                                                        let! _ =
                                                            vectorDbService.StoreEmbeddingAsync(
                                                                embeddingId,
                                                                tenantId,
                                                                book.BookId,
                                                                embeddingResult,
                                            ...
                                            return ()
                                        | None -> return! Error "Metadata not found"
                                }

...
type TenantEvent =
    ...
    interface Event<Tenant> with
        member this.Process(tenant: Tenant) : Result<Tenant, string> =
            ...
            | ScheduledForDeletion date -> tenant.ScheduleForDeletion date
            | TagAdded tag -> tenant.AddTag tag
            | TagRemoved tag -> tenant.RemoveTag tag
            | TagReplaced(oldTag, newTag) -> tenant.ReplaceTag(oldTag, newTag)
            | PatronAdded(userId, role) -> tenant.AddPatron(userId, role)
            | PatronDemoted userId -> tenant.DemotePatron userId
            | PatronPromoted userId -> tenant.PromotePatron userId
            | PatronRemoved userId -> tenant.RemovePatron userId
            | PatronInvited(userId, invitationCode) -> tenant.InvitePatron(userId, invitationCode)
            | InvitedPatronConvertedToPatron invitationCode -> tenant.ConvertInvitedPatronToPatron invitationCode
            | PatronSuspended(userId, reason) -> tenant.SuspendPatron(userId, reason)
            | PatronReadmitted userId -> tenant.ReAdmittPatron userId
            ...
            | JoinRequestSubmitted2 userId -> tenant.AddJoinRequest2 userId
            | JoinRequestApproved2 userId -> tenant.ApproveJoinRequest2 userId
            | JoinRequestRejected2 userId -> tenant.RejectJoinRequest2 userId

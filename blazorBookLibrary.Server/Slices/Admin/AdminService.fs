...
type AdminService
    ...
    member this.PurgeVectorsReferringDroppedBooksAsync(context: UserContext, ?ct) =
        ...
        taskResult {
            ...
            let unexistingBookReferedBookIds =
                results |> Array.filter snd |> Array.map fst |> Array.toList
    ...
    member this.AdjustBookStatesReferringMissingEmbeddingsAsync(context: UserContext, ?ct) =
        ...
        taskResult {
            ...
            let bookIdsEmbeddingIds =
                booksWithEmbeddings |> List.map (fun b -> b.Id, b.OptionalEmbedding.Value)
    ...
    member this.AssignUserToDistributionPointAsync
        (context: UserContext, id: DistributionPointId, userId: UserId, ?ct: CancellationToken)
        =
        taskResult {
            let ct = defaultArg ct CancellationToken.None
            do! checkIsGlobalAdminOrTenantManager context ct
    ...
    member this.UnassignUserFromDistributionPointAsync
        (context: UserContext, id: DistributionPointId, userId: UserId, ?ct: CancellationToken)
        =
        taskResult {
            let ct = defaultArg ct CancellationToken.None
    ...
    member this.UpdateDistributionPointInfoAsync
        (context: UserContext, id: DistributionPointId, info: Info, ?ct: CancellationToken)
        =
        taskResult {
            let ct = defaultArg ct CancellationToken.None
            do! checkIsGlobalAdminOrTenantManager context ct
    ...
    member this.RenameDistributionPointAsync
        (context: UserContext, id: DistributionPointId, name: NonEmptyName, ?ct: CancellationToken)
        =
        taskResult {
            let ct = defaultArg ct CancellationToken.None
            do! checkIsGlobalAdminOrTenantManager context ct
        ...
        taskResult {
            ...
            for (bookId, group) in duplicates do
                ...
                | Some embeddingId ->
                    ...
                    let toRemove =
                        group |> Seq.map fst |> Seq.filter (fun id -> id <> embeddingId) |> Seq.toList

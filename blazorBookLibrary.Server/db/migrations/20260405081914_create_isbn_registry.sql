...
CREATE TABLE public.aggregate_events_01_IsbnRegistry (
                                                    id integer DEFAULT nextval('public.aggregate_events_01_IsbnRegistry_id_seq') NOT NULL,
                                                    aggregate_id uuid NOT NULL,
...
create index ix_01_events_IsbnRegistry_id on public.events_01_IsbnRegistry(aggregate_id);
create index ix_01_aggregate_events_IsbnRegistry_id on public.aggregate_events_01_IsbnRegistry(aggregate_id);
create index ix_01_snapshot_IsbnRegistry_id on public.snapshots_01_IsbnRegistry(aggregate_id);
create index ix_01_snapshot_IsbnRegistry_aggregate_id_and_id on public.snapshots_01_IsbnRegistry(aggregate_id, id DESC);
create index ix_01_snapshot_IsbnRegistry_event_id on public.snapshots_01_IsbnRegistry(event_id);
create index ix_01_events_IsbnRegistry_timestamp on public.events_01_IsbnRegistry("timestamp");
create index ix_01_snapshots_IsbnRegistry_timestamp on public.snapshots_01_IsbnRegistry("timestamp");
                                                                                                                                                          
CREATE OR REPLACE FUNCTION insert_01_IsbnRegistry_event_and_return_id(
    IN event_in text,
...
CREATE OR REPLACE FUNCTION insert_md_01_IsbnRegistry_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
...
CREATE OR REPLACE FUNCTION insert_md_01_IsbnRegistry_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN distance_from_latest_snapshot int,
    IN md text   
...
CREATE OR REPLACE FUNCTION insert_md_01_IsbnRegistry_aggregate_event_and_return_id_opt_lock(
    IN event_in text,
    IN aggregate_id uuid,
    IN distance_from_latest_snapshot int,
    IN md text,   

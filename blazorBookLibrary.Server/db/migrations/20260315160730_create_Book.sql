...
CREATE TABLE public.aggregate_events_01_Book (
                                                    id integer DEFAULT nextval('public.aggregate_events_01_Book_id_seq') NOT NULL,
                                                    aggregate_id uuid NOT NULL,
...
create index ix_01_events_Book_id on public.events_01_Book(aggregate_id);
create index ix_01_aggregate_events_Book_id on public.aggregate_events_01_Book(aggregate_id);
create index ix_01_snapshot_Book_id on public.snapshots_01_Book(aggregate_id);
create index ix_01_snapshot_Book_aggregate_id_and_id on public.snapshots_01_Book(aggregate_id, id DESC);
create index ix_01_snapshot_Book_event_id on public.snapshots_01_Book(event_id);
create index ix_01_events_Book_timestamp on public.events_01_Book("timestamp");
create index ix_01_snapshots_Book_timestamp on public.snapshots_01_Book("timestamp");
                                                                                                                                                          
CREATE OR REPLACE FUNCTION insert_01_Book_event_and_return_id(
    IN event_in text,
...
CREATE OR REPLACE FUNCTION insert_md_01_Book_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
...
CREATE OR REPLACE FUNCTION insert_01_Book_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid 
)
RETURNS int
...
CREATE OR REPLACE FUNCTION insert_md_01_Book_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN distance_from_latest_snapshot int,
    IN md text   
...
CREATE OR REPLACE FUNCTION insert_md_01_Book_aggregate_event_and_return_id_opt_lock(
    IN event_in text,
    IN aggregate_id uuid,
    IN distance_from_latest_snapshot int,
    IN md text,   

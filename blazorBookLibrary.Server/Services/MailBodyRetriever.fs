...
type MailBodyRetriever() =
    ...
    interface IMailBodyRetriever with
        member this.GetLoanNotificationTextMailAsync (bookTitle: Title, tenantName: TenantName, distributionPoint: string, loanDate: DateTime, dueDate: DateTime, shortLang: ShortLang, ?ct:CancellationToken) =
            ...
            taskResult {
                ...
                let contentReplaced =
                    content.Replace("{bookTitle}", bookTitle.Value)
        ...
        member this.GetLoanNotificationSubject (bookTitle: Title, dueDate: DateTime, shortLang, ?ct:CancellationToken) = 
            let templatePath = System.IO.Path.Combine (AppDomain.CurrentDomain.BaseDirectory, "Templates", shortLang.Value, "LoanNotificationSubject.txt")
            let ct = defaultArg ct CancellationToken.None
            taskResult {
                let! content = System.IO.File.ReadAllTextAsync (templatePath, ct)
                let contentReplaced =
        ...
        member this.GetReleaseLoanNotificationTextMailAsync (userName: string, bookTitle: Title, loanedAt, returnedAt, tenantName: TenantName, dpName: NonEmptyName, shortLang, ?ct:CancellationToken) =
        ...
        member this.GetReleaseLoanNotificationSubject (bookTitle: Title, shortLang: ShortLang, ?ct:CancellationToken) = 
            let templatePath = System.IO.Path.Combine (AppDomain.CurrentDomain.BaseDirectory, "Templates", shortLang.Value, "LoanReturnNotificationSubject.txt")
            let ct = defaultArg ct CancellationToken.None
            taskResult {
                let! content = System.IO.File.ReadAllTextAsync (templatePath, ct)
                let contentReplaced =
        ...
        member this.GetReservationNotificationTextMailAsync (bookTitle: Title, code: ReservationCode, tenantName: TenantName, distributionPoint: string, shortLang, ?ct:CancellationToken) = 
        ...
        member this.GetPatronInvitationSubject(tenantName: TenantName, userName: string, shortLang: ShortLang, ?ct: CancellationToken) = 
            ...
            taskResult {
                let! content = System.IO.File.ReadAllTextAsync (templatePath, ct)
                let contentReplaced =
                    content.Replace("{tenantName}", tenantName.Value)

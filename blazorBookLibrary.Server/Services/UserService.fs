...
type UserService
    ...
    member this.CreateUserAsync(_: UserContext, user: User, ?ct: CancellationToken) : Task<Result<unit, string>> =
        taskResult {
            let result = runInitAsync<User, UserEvent, string> eventStore messageSenders user ct
            return! result
        }

    member this.GetUserAsync
    ...
    member this.GetUserDetailsAsync
        (context: UserContext, userId: UserId, ?ct: CancellationToken)
    ...
    member private this.UpdateAppUserPropertyAsync
        (context: UserContext, userId: UserId, updateAction: ApplicationUser -> unit, ?ct: CancellationToken)
            ...
            let! appUser =
                task {
                    ...
                    if user <> null then
                        ...
                        return Error(sprintf "User %s not found" userIdStr)
                }
            ...
            let! updateResult =
                task {
                    ...
                    if result.Succeeded then
                        ...
                        return Error(result.Errors |> Seq.map (fun e -> e.Description) |> String.concat ", ")
                }
    ...
    member private this.UpdateAppUserAndAggregateAsync
        (
            context: UserContext,
            userId: UserId,
            updateAction: ApplicationUser -> unit,
            command: UserCommand,
    ...
    member this.SetFiscalCodeAsync
        (context: UserContext, userId: UserId, fiscalCode: FiscalCode, ?ct: CancellationToken)
        : Task<Result<unit, string>> =
        let ct = defaultArg ct CancellationToken.None
        ...
        taskResult {
            ...
            let! result =
                this.UpdateAppUserAndAggregateAsync(
                    context,
                    userId,
                    (fun u -> u.CodiceFiscale <- fiscalCode.Value),
    ...
    member this.SetNameAsync
        (context: UserContext, userId: UserId, name: string, ?ct: CancellationToken)
        : Task<Result<unit, string>> =
        this.UpdateAppUserAndAggregateAsync(context, userId, (fun u -> u.Nome <- name), SetNome name, ?ct = ct)

    member this.SetSurnameAsync
        (context: UserContext, userId: UserId, surname: string, ?ct: CancellationToken)
        : Task<Result<unit, string>> =
        this.UpdateAppUserAndAggregateAsync(
            context,
            userId,
            (fun u -> u.Cognome <- surname),
            SetCognome surname,
            ?ct = ct
        )

    member this.SetPhoneNumberAsync
        (context: UserContext, userId: UserId, phoneNumber: PhoneNumber, ?ct: CancellationToken)
        : Task<Result<unit, string>> =
        this.UpdateAppUserAndAggregateAsync(
            context,
            userId,
            (fun u -> u.PhoneNumber <- phoneNumber.Value),
            SetPhoneNumber phoneNumber,
            ?ct = ct
        )

    member this.SetIsPhysicallyIdentifiedAsync
        (context: UserContext, userId: UserId, ?ct: CancellationToken)
        : Task<Result<unit, string>> =
        let ct = defaultArg ct CancellationToken.None
        ...
        taskResult {
            ...
            return!
                this.UpdateAppUserAndAggregateAsync(
                    context,
                    userId,
                    (fun u -> u.IsIdentifiedPhysically <- true),
                    SetPhysicalIdentification,
                    ct
                )
        }

    member this.UnSetIsPhysicallyIdentifiedAsync
        (context: UserContext, userId: UserId, ?ct: CancellationToken)
        : Task<Result<unit, string>> =
        this.UpdateAppUserAndAggregateAsync(
            context,
            userId,
            (fun u -> u.IsIdentifiedPhysically <- false),
            UnsetPhysicalIdentification,
            ?ct = ct
        )

    member this.GhostUserAsync
        (context: UserContext, userId: UserId, ?ct: CancellationToken)
        : Task<Result<unit, string>> =
        taskResult {
    ...
    member this.SetAppUserInfoAsync
        (context: UserContext, userId: UserId, appUserInfo: AppUserInfo, ?ct: CancellationToken)
        : Task<Result<unit, string>> =
        let ct = defaultArg ct CancellationToken.None
    ...
    member this.GetDistributionPointsManagedByUserAsync
        (context: UserContext, userId: UserId, ?ct: CancellationToken)
    ...
    member this.SetCurrentTenantAsync
        (context: UserContext, userId: UserId, tenantId: TenantId, ?ct: CancellationToken)
        =
        taskResult {
            let ct = defaultArg ct CancellationToken.None
            ...
            let! (eventId, user) =
                getAggregateStorageFreshStateViewerAsync<User, UserEvent, string> eventStore (Some ct) userId.Value
            ...
            do!
                if not allowed then
                    Error "Not allowed to set current tenant"
                else
                    Ok()
    ...
    member this.SetAppUserInfoUnsafeAsync
        (userId: UserId, appUserInfo: AppUserInfo, ?ct: CancellationToken)
        : Task<Result<unit, string>> =
        let ct = defaultArg ct CancellationToken.None
    ...
    member this.GetUserIdByEmailAsync
        (context: UserContext, email: string, ?ct: CancellationToken)
        : Task<Result<UserId, string>> =
        let ct = defaultArg ct CancellationToken.None

        taskResult {
            match context with
            | UserContext.Anonymous -> return! Error "Anonymous users are not allowed to search users"
                ...
                let! appUser =
                    task {
                        ...
                        if user <> null then
                            ...
                            return Error(sprintf "User with email %s not found" email)
                    }

                return UserId(Guid.Parse(appUser.Id))
        }

    member this.SetLangPref
        (context: UserContext, userId: UserId, langPref: ShortLang, ?ct: CancellationToken)
        : Task<Result<unit, string>> =
        let ct = defaultArg ct CancellationToken.None
    ...
    new
        (
            ...
            scopeFactory: IServiceScopeFactory,
            secretsReader: BookLibrary.Utils.SecretsReader,
            reviewService: IReviewService,
            userTenantResolverService: IUserTenantResolverService,
            logger: ILogger<UserService>
        ) =
    ...
    interface IUserService with
        ...
        member this.SetFiscalCodeAsync
            (context, userId: UserId, fiscalCode: FiscalCode, ?ct: CancellationToken)
            : Task<Result<unit, string>> =
            let ct = defaultArg ct CancellationToken.None
            this.SetFiscalCodeAsync(context, userId, fiscalCode, ct)

        member this.SetNameAsync
            (context, userId: UserId, name: string, ?ct: CancellationToken)
            : Task<Result<unit, string>> =
            let ct = defaultArg ct CancellationToken.None
            this.SetNameAsync(context, userId, name, ct)

        member this.SetSurnameAsync
            (context, userId: UserId, surname: string, ?ct: CancellationToken)
            : Task<Result<unit, string>> =
            let ct = defaultArg ct CancellationToken.None
            this.SetSurnameAsync(context, userId, surname, ct)

        member this.SetPhoneNumberAsync
            (context, userId: UserId, phoneNumber: PhoneNumber, ?ct: CancellationToken)
            : Task<Result<unit, string>> =
            let ct = defaultArg ct CancellationToken.None
            this.SetPhoneNumberAsync(context, userId, phoneNumber, ct)

        member this.SetIsPhysicallyIdentifiedAsync
            (context, userId: UserId, ?ct: CancellationToken)
            : Task<Result<unit, string>> =
            let ct = defaultArg ct CancellationToken.None
        ...
        member this.SetAppUserInfoAsync
            (context, userId: UserId, appUserInfo: AppUserInfo, ?ct: CancellationToken)
            : Task<Result<unit, string>> =
            let ct = defaultArg ct CancellationToken.None
            this.SetAppUserInfoAsync(context, userId, appUserInfo, ct)

        member this.SetLangPrefAsync
            (context: UserContext, userId: UserId, langPref: ShortLang, ?ct: CancellationToken)
            : Task<Result<unit, string>> =
            let ct = defaultArg ct CancellationToken.None
        ...
        member this.SetCurrentTenantAsync
            (context, userId: UserId, tenantId: TenantId, ?ct: CancellationToken)
            : Task<Result<unit, string>> =
            let ct = defaultArg ct CancellationToken.None
            this.SetCurrentTenantAsync(context, userId, tenantId, ct)

        member this.GetUserIdByEmailAsync
            (context, email: string, ?ct: CancellationToken)
            : Task<Result<UserId, string>> =
            let ct = defaultArg ct CancellationToken.None

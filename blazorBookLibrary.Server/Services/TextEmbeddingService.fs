...
type TextEmbeddingService
    ...
    interface ITextEmbeddingService with
        ...
        member this.GetBookDescriptionAsync(bookData: PartialBookDataMatch, [<Optional; DefaultParameterValue(null)>] ?ct: CancellationToken) =
            let ct = defaultArg ct CancellationToken.None
            task {
                try
                    let modelName = "gemini-2.5-flash-lite"
        ...
        member this.GetPartialBookMatchByCoverImage (base64Image: string, mimeType: string, [<Optional; DefaultParameterValue(null)>] ?ct: CancellationToken) =
            let ct = defaultArg ct CancellationToken.None
            task {
                try

            ...
            taskResult {
                try
                    ...
                    if not response.IsSuccessStatusCode then
                        ...
                        let result =
                            JsonSerializer.Deserialize<GoogleGenerateResponse>(jsonResponse, options)

                        if Object.ReferenceEquals(result, null) then
                            return! Error "Failed to deserialize Gemini response."
                        elif
                            not (isNull result.candidates)
                    ...
                    if not response.IsSuccessStatusCode then
                        ...
                        then
                            ...
                            try
                                ...
                                | true, false, false -> return (AuthorAndIsbn(isbn, [])) // Should probably be All or something but let's stick to DU

            ...
            taskResult {
                try
                    ...
                    if not response.IsSuccessStatusCode then
                        ...
                        return { Model = modelName; Vector = result.embedding.values }
                with
            ...
            taskResult {
                ...
                try
                    ...
                    let bookInfo = 
                        ...
                        | TitleAndIsbn (title, isbn) -> 
                            sprintf "Title: %s, ISBN: %s" title isbn
                    ...
                    let prompt = $"Provide a short, engaging description (max 150 words) for the following book to be used in a library catalogue. Focus on the plot summary or main themes. Use the same language as the book title if possible.\n\nBook Info: {bookInfo}\n\nDescription:"
                    ...
                    if not response.IsSuccessStatusCode then
                        ...
                        if Object.ReferenceEquals(result, null) then
                            ...
                            return! Error "No description generated by the model."
                with
                | ex -> return! Error ex.Message
            }
                        ...
                        if Object.ReferenceEquals(result, null) || Object.ReferenceEquals(result.candidates, null) || result.candidates.Length = 0 || 
                           Object.ReferenceEquals(result.candidates.[0].content, null) || Object.ReferenceEquals(result.candidates.[0].content.parts, null) || result.candidates.[0].content.parts.Length = 0 then
                            ...
                            try
                                ...
                                | true, true, true -> 
                                    return (All (isbn, title, authors))
                                | true, true, false -> 
                                    return (TitleAndIsbn (title, isbn))
                                | true, false, true -> 
                                    return (AuthorAndIsbn (isbn, authors))
                                | true, false, false -> 
                                    return (AuthorAndIsbn (isbn, [])) // Should probably be All or something but let's stick to DU
                                | false, true, true -> 
                                    return (TitleAndAuthor (title, authors))

...
with 
    ...
    member this.AddReferenceUser (referenceUser: UserId) = 
        if this.ReferenceUsers |> List.contains referenceUser then
            ...
            { this with ReferenceUsers = referenceUser :: this.ReferenceUsers } |> Ok
    ...
    member this.RemoveReferenceUser (referenceUser: UserId) = 
        if this.ReferenceUsers |> List.contains referenceUser |> not then
            ...
            { this with ReferenceUsers = this.ReferenceUsers |> List.filter (fun x -> x <> referenceUser) } |> Ok
    
    member this.Rename (name: NonEmptyName) = 
        { this with Name = name } |> Ok
        
    member this.UpdateInfo (info: Info) = 
        { this with Info = info } |> Ok

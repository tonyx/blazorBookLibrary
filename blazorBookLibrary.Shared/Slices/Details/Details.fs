...
module Details =
    ...
    type UserDetails =
        { User: User
          ...
          RefarenceOfDistributionPoints: List<DistributionPoint> }

        member this.HasReservedBook(bookId: BookId) =
            this.FutureReservations
            |> List.exists (fun (reservation, _) -> reservation.BookId = bookId)

        member this.HasAnApprovedReviewOfBook(bookId: BookId) =
            this.BooksAndReviews
            |> List.exists (fun (_, review) -> review.BookId = bookId && review.ApprovalStatus.IsApproved)

        member this.HasAHiddenReviewOfBook(bookId: BookId) =
            this.BooksAndReviews
            |> List.exists (fun (_, review) -> review.BookId = bookId && review.Hidden)
        ...
        member this.ToLoan(now: DateTime) =
            if now > this.Reservation.TimeSlot.End then
                ...
                if (now > this.Reservation.TimeSlot.Start) then
                    ...
                    { loan with
                        TimeSlot = this.Reservation.TimeSlot.Shift now }
                    |> Ok
    ...
    type BookDetails =
        ...
        member this.GetNextAvailableTimeSlot(timeSlotLoanDurationInDays: int, now: DateTime) =
            let currentTimeSlots =
                (if this.CurrentLoan.IsSome then
                     [ this.CurrentLoan.Value.Loan.TimeSlot ]
                 else
                     [])
